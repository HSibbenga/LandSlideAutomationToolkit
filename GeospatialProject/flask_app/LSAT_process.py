...
#
##
def process_files(raster_path, shp_path, higher_res_path, landslide_analysis_choice):
    try:
        ...
        if higher_res_path:
            ...
            # Log progress
            logging.info("Raster upsampling and CSV conversion completed")

            # Perform Landslide Population Analysis and export results to CSV
            if landslide_analysis_choice:
                logging.info("Landslide Population Analysis started")
                ...
                logging.info(f"Landslide Population Analysis completed. Results exported to: {output_population_risk_path}")
                return f"Landslide Population Analysis completed. Results exported to: {output_population_risk_path}"
            else:
                # If the user chooses not to perform the analysis, log a message indicating the choice
                logging.info("Landslide Population Analysis was skipped.")
                return "Landslide Population Analysis was skipped."
        else:
            logging.info(f"Raster masking completed. Masked raster saved at: {output_masked_path}")
            return f"Raster masking completed. Masked raster saved at: {output_masked_path}" 
